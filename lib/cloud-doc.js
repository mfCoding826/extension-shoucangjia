...
async function addRecord(fields) {
  ...
  const tableName = config.feishu_table_name;

  if (!appToken || !tableName) {
    throw new Error('飞书表格配置不完整：缺少多维表格 ID 或数据表名称');
  }

  // 首先需要获取 table_id（通过 table_name 查找）
  const tableId = await getTableId(token, appToken, tableName);

  // 构建请求体
  // 飞书的 fields 格式：{ "字段名": "值" }
  // 注意：字段名必须与表格中实际创建的列名完全一致
  const body = {
    fields: {
      '页面类型': fields.page_type || '',
      '页面URL': fields.page_url || '',
      '页面内容总结': fields.page_summary || ''
    }
  };
